...
```{r}
alz = read.csv("D:/ADNI/bivariate_fix.csv")
...
Transpose the table and write to .csv. Some manual work here with .csv file. This included deleting the first row in the table labeled "V1, V2, V3..."

```{r}
alz = t(alz)
alz = as.data.frame(alz)
write.csv(alz, "single_snps_alz_transpose(2).csv")
```

Read in the transposed table to run glm. Delete the first 3 rows of the new table (these are useful but cannot be run with the glm). Using excel, make sure to delete any columns with 'NA' in any of the 'APOE', 'Diagnosis', or 'Gender' columns.

```{r}
alz = read.csv("single_snps_alz_transpose(2).csv", stringsAsFactors = FALSE)
alz = alz[-1,]
alz = alz[-1,]
alz = alz[-1,]
alz = alz[-1,] #x4
...
if (missing1valuecounter > 1) {
  df2 = list.cbind(newlist_2)

}
if (missing2valuecounter > 1) {
...
filtered_table = read.csv('D:/ADNI/single_snps_alz_transpose(2).csv')
...
It gets tricky when there is greater than one alt allele. The code below will adjust for 2 alt alleles, but anything more might need some manual revision. This is fine with smaller datasets like the one we're working with, but with larger genomes we might want to revisit this. 
...
write.csv(finaltable, 'single_snp_pvalues2.csv')
